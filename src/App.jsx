...
Return exactly:
{"text":"clean task title","section":"deals|projects|admin|kids|house|travel|general","tab":"work|personal","status":"waiting_on_me|waiting_on_others|on_hold|done","priority":"high|medium|low","dueDate":"YYYY-MM-DD or empty","notes":"extra context or empty"}
...
function StatusBadge({ status, small, onClick }) {
  ...
  return (
    <span onClick={onClick} style={{
      ...
      whiteSpace: "nowrap", userSelect: "none", cursor: onClick ? "pointer" : "default",
...
function PriorityDot({ priority, onClick }) {
  ...
  return (
    <span onClick={onClick} style={{
      ...
      background: p.color, flexShrink: 0, cursor: onClick ? "pointer" : "default",
    }} title={p.label} />
  );
}
...
function Pill({ label, active, color, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      transition: "all 0.15s", whiteSpace: "nowrap", WebkitTapHighlightColor: "transparent",
    }}>{label}</button>
  );
}
...
function RichToolbar({ editorRef, accent }) {
  ...
  const btn = (active) => ({
    padding: "4px 8px", borderRadius: 5, border: `1px solid ${active ? accent : "#e5e7eb"}`,
    background: active ? accent : "white", color: active ? "white" : "#374151",
    cursor: "pointer", fontSize: 12, fontFamily: "inherit", lineHeight: 1.4,
    fontWeight: active ? 700 : 400, minHeight: 28, WebkitTapHighlightColor: "transparent",
  });
    ...
    <div style={{ position: "relative" }}>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 3, alignItems: "center", padding: "6px 8px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
        <select onChange={e => exec("fontName", e.target.value)} defaultValue=""
          style={{ fontSize: 11, padding: "3px 5px", borderRadius: 5, border: "1px solid #e5e7eb", fontFamily: "inherit", color: "#374151", background: "white", height: 28 }}>
          <option value="" disabled>Font</option>
          {FONTS.map(f => <option key={f} value={f==="Default"?"inherit":f}>{f}</option>)}
        </select>
        <select onChange={e => { exec("fontSize","7"); const el=editorRef.current?.querySelector('font[size="7"]'); if(el){el.removeAttribute("size");el.style.fontSize=e.target.value;} }} defaultValue=""
          style={{ fontSize: 11, padding: "3px 5px", borderRadius: 5, border: "1px solid #e5e7eb", fontFamily: "inherit", color: "#374151", background: "white", height: 28, width: 60 }}>
        ...
        <div style={{ display: "flex", gap: 2 }}>
          {TEXT_COLORS.map(c => (
            <button key={c} onMouseDown={e=>{e.preventDefault();exec("foreColor",c);}}
              style={{ width: 18, height: 18, borderRadius: 3, background: c, border: "1.5px solid rgba(0,0,0,0.12)", cursor: "pointer", padding: 0, flexShrink: 0 }} />
          ))}
        </div>
        <div style={sep} />
        <button onMouseDown={e=>{e.preventDefault();exec("insertUnorderedList");}} style={btn(isActive("insertUnorderedList"))} title="Bullet list">≡</button>
        <button onMouseDown={e=>{e.preventDefault();exec("insertOrderedList");}} style={btn(isActive("insertOrderedList"))} title="Numbered list">1.</button>
        <div style={sep} />
      ...
      {showLink && (
        <div style={{ position:"absolute", top:"100%", left:0, zIndex:50, background:"white", border:"1px solid #e5e7eb", borderRadius:8, padding:"8px 10px", boxShadow:"0 4px 16px rgba(0,0,0,0.1)", display:"flex", gap:6, alignItems:"center", minWidth:260, width:"100%" }}>
          <input autoFocus value={linkUrl} onChange={e=>setLinkUrl(e.target.value)}
            ...
            placeholder="https://…"
            style={{ flex:1, fontSize:12, padding:"5px 8px", borderRadius:6, border:"1px solid #e5e7eb", outline:"none", fontFamily:"inherit", color:"#111" }} />
...
function AddForm({ onAdd, accent, isSubtask }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{
      width:"100%", padding: isSubtask ? "6px 10px" : "9px 12px",
      borderRadius:8, border:`1.5px dashed ${accent}55`,
      background:`${accent}08`, color:accent,
      fontSize: isSubtask ? 12 : 13, fontWeight:600,
      cursor:"pointer", textAlign:"left", fontFamily:"inherit",
      WebkitTapHighlightColor:"transparent",
    }}>+ {isSubtask ? "Add subtask" : "Add task"}</button>
  );

  return (
    <div className="fade-in" style={{ background:"white", borderRadius:10, border:`1.5px solid ${accent}44`, overflow:"hidden", boxShadow:`0 4px 20px ${accent}14` }}>
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center", padding:"8px 10px", borderTop:"1px solid #f3f4f6", background:"#fafafa" }}>
        <select value={status} onChange={e=>setStatus(e.target.value)}
          style={{ fontSize:11, padding:"5px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151", background:"white" }}>
        ...
        <input type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)}
          style={{ fontSize:11, padding:"5px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151" }} />
...
function TaskCard({ task, onUpdate, onDelete, accent }) {
  ...
  return (
    <div className="fade-in" style={{ background:"white", borderRadius:12, border:`1px solid ${isDone?"#f3f4f6":"rgba(0,0,0,0.08)"}`, marginBottom:8, overflow:"hidden", opacity:isDone?0.65:1, transition:"opacity 0.2s" }}>
      <div style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"13px 14px" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          {editing ? (
            ...
            <div onDoubleClick={()=>!task.synced&&setEditing(true)}
              dangerouslySetInnerHTML={{__html:task.text}}
        ...
        <div style={{ borderTop:"1px solid #f3f4f6", padding:"10px 14px 12px" }}>
          ...
          {!task.synced && (
            <textarea value={editNotes} onChange={e=>setEditNotes(e.target.value)}
              onBlur={()=>onUpdate({...task,notes:editNotes})} placeholder="Notes…"
...
function VoiceButton({ onTaskCreated, currentTab }) {
  ...
  useEffect(()=>{
    if (!listening && liveText && phase==="listening") {
      ...
      parseVoiceWithClaude(liveText, currentTab, today)
        .then(r=>{ setParsed(r); setPhase("preview"); })
    ...
    <>
      <button onClick={()=>{ if(!open){setOpen(true);return;} if(phase==="idle"||phase==="error"){setPhase("listening");start();} else if(phase==="listening"){stop();} }}
        style={{
          ...
          background:phase==="listening"?"#dc2626":ac,
          border:"none", cursor:"pointer", color:"white",
          fontSize:22, display:"flex", alignItems:"center", justifyContent:"center",
          boxShadow:"0 4px 20px rgba(0,0,0,0.25)", transition:"background 0.2s",
          animation: phase==="listening" ? `${currentTab==="work"?"voicePulse":"voicePulseGreen"} 1s ease-in-out infinite` : "none",
          WebkitTapHighlightColor:"transparent",
        }}>
        {phase==="processing" ? <span style={{ fontSize:18, animation:"spin 0.8s linear infinite", display:"inline-block" }}>⟳</span>
         : phase==="listening" ? "⏹" : "🎙️"}
      </button>

      {open && (
        <div style={{
          position:"fixed", bottom:90, right:12, left:12, maxWidth:360, margin:"0 auto",
          zIndex:99, background:"white", borderRadius:16,
          border:"1px solid rgba(0,0,0,0.1)", boxShadow:"0 8px 40px rgba(0,0,0,0.18)",
          ...
          <div style={{ padding:"12px 14px", background:ac, color:"white", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
            <span style={{ fontWeight:700, fontSize:13 }}>
              {phase==="idle"&&"🎙️ Voice Task"}
              {phase==="listening"&&"🔴 Listening…"}
              {phase==="processing"&&"✨ AI parsing…"}
              {phase==="preview"&&"✅ Review task"}
              {phase==="error"&&"❌ Couldn't parse"}
            </span>
          ...
          <div style={{ padding:14 }}>
            ...
            {phase==="listening" && (
              <div>
                <div style={{ minHeight:64, padding:"10px 12px", background:"#f9fafb", borderRadius:8, border:"1px solid #e5e7eb", fontSize:13, color:liveText?"#111":"#9ca3af", fontStyle:liveText?"normal":"italic", lineHeight:1.5, marginBottom:10 }}>
                  {liveText||"Speak now…"}
                </div>
            ...
            {phase==="preview" && parsed && (
              <div>
                <div style={{ marginBottom:10 }}>
                  ...
                  <input value={parsed.text} onChange={e=>editParsed("text",e.target.value)}
                    style={{ width:"100%", fontSize:13, padding:"7px 9px", borderRadius:7, border:"1px solid #e5e7eb", outline:"none", fontFamily:"inherit", color:"#111" }} />
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
                  <div>
                    <label style={{ fontSize:11, fontWeight:700, color:"#9ca3af", display:"block", marginBottom:3 }}>SECTION</label>
                    <select value={parsed.section} onChange={e=>editParsed("section",e.target.value)}
                      style={{ width:"100%", fontSize:11, padding:"6px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151", background:"white" }}>
                  ...
                  <div>
                    ...
                    <select value={parsed.status} onChange={e=>editParsed("status",e.target.value)}
                      style={{ width:"100%", fontSize:11, padding:"6px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151", background:"white" }}>
                      {STATUSES.filter(s=>s.id!=="all").map(s=><option key={s.id} value={s.id}>{s.label}</option>)}
                    </select>
                  </div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
                  <div>
                    ...
                    <select value={parsed.priority} onChange={e=>editParsed("priority",e.target.value)}
                      style={{ width:"100%", fontSize:11, padding:"6px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151", background:"white" }}>
                      {PRIORITIES.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <input type="date" value={parsed.dueDate} onChange={e=>editParsed("dueDate",e.target.value)}
                      style={{ width:"100%", fontSize:11, padding:"6px 7px", borderRadius:6, border:"1px solid #e5e7eb", fontFamily:"inherit", color:"#374151" }} />
                  </div>
                </div>
                {parsed.notes && (
                  <div style={{ marginBottom:10, padding:"7px 9px", background:"#fffbeb", borderRadius:7, border:"1px solid #fde68a", fontSize:11, color:"#92400e" }}>💡 {parsed.notes}</div>
                )}
...
export default function App() {
  ...
  useEffect(()=>{
    try { const s=localStorage.getItem("joan_todos_v3"); if(s){setTasks(JSON.parse(s));return;} } catch {}
    const init={}; [...WORK_SECTIONS,...PERSONAL_SECTIONS].forEach(s=>{init[s.id]=[];}); setTasks(init);
  },[]);
  useEffect(()=>{ if(Object.keys(tasks).length>0) localStorage.setItem("joan_todos_v3",JSON.stringify(tasks)); },[tasks]);
  ...
  const syncDeals = useCallback(async()=>{
    ...
    try {
      ...
      setTasks(prev=>({...prev,deals:[...(prev.deals||[]).filter(t=>!t.synced),...dealTasks]}));
  ...
  const updateSectionTasks = (sid, updated) => {
    setTasks(prev=>({...prev,[sid]:updated}));
  ...
  const handleVoiceTask = useCallback((parsed)=>{
    ...
    setTasks(prev=>({...prev,[sid]:[...(prev[sid]||[]),{id:genId(),text:parsed.text,status:parsed.status||"waiting_on_me",priority:parsed.priority||"medium",dueDate:parsed.dueDate||"",notes:parsed.notes||"",subtasks:[],isRich:false}]}));
    ...
    <div style={{ minHeight:"100vh", background:tab==="work"?"linear-gradient(160deg,#f0f0ff 0%,#f8f7ff 60%,#faf9ff 100%)":"linear-gradient(160deg,#f0fdf4 0%,#f7fef9 60%,#fafff8 100%)", transition:"background 0.4s", paddingBottom:88 }}>
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"14px 12px 16px" }}>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          {showFilters&&(
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", animation:"fadeIn 0.15s ease-out" }}>
              {STATUSES.map(s=>(
        ...
        <p style={{ fontSize:11, color:"#b0b5be", textAlign:"center", marginTop:20, lineHeight:1.8 }}>
          Saved locally · Double-click tasks to edit · Tap badges to cycle status/priority
        </p>
      </div>
